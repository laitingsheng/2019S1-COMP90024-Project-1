...
def process_line(line):
	...
	x_coor, y_coor, hashtags_data = float(result.groups()[0]), float(result.groups()[1]), result.groups()[2]
...
def processTwitterFile(data):
	file, start_point, end_point = open(TFPATH, "r", encoding='UTF-8'), data[0], data[1]	
...
if __name__ == '__main__':
	...
	process_list = [(file_length_per_core * i, file_length_per_core * (i + 1)) for i in range(core_num)]
	...
	twitter_count_order = sorted(grid_twit_count.items(), key = lambda x: x[1], reverse = True)
	...
	for grid in grid_hashtag_count:
		top5hashtags = sorted(grid_hashtag_count[grid].items(), key = lambda x: x[1], reverse = True)[:5]
